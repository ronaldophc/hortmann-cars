<footer class="footer sm:footer-horizontal bg-base-200 text-base-content p-10 shadow-inner">
    <aside>
        <a href="#">
            <img class="h-25 w-auto" src="{{ asset('images/logo.png') }}" alt="">
        </a>

    </aside>
    <nav>
        <h6 class="footer-title text-lg">Mapa do Site</h6>
        <a class="link link-hover text-md">Nossos carros</a>
        <a class="link link-hover text-md">Contato</a>
    </nav>
    <nav>
        <h6 class="footer-title text-lg">Telefones</h6>
        <a class="link link-hover text-md">{{ $settings->phone_1 }}</a>
        <a class="link link-hover text-md">{{ $settings->phone_2 }}</a>
        <h6 class="footer-title mt-2 text-lg">Horários</h6>
        <a class="link link-hover text-md">{{ $settings->opening_hours }}</a>
    </nav>
    <nav>
        <h6 class="footer-title text-lg">Redes Sociais</h6>
        <div class="flex">
            @if ($settings->instagram_url)
                <a href="{{ $settings->instagram_url }}" target="_blank" class="link link-hover"
                    aria-label="Instagram">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 fill-current" viewBox="0 0 30 30">
                        <path
                            d="M 9.9980469 3 C 6.1390469 3 3 6.1419531 3 10.001953 L 3 20.001953 C 3 23.860953 6.1419531 27 10.001953 27 L 20.001953 27 C 23.860953 27 27 23.858047 27 19.998047 L 27 9.9980469 C 27 6.1390469 23.858047 3 19.998047 3 L 9.9980469 3 z M 22 7 C 22.552 7 23 7.448 23 8 C 23 8.552 22.552 9 22 9 C 21.448 9 21 8.552 21 8 C 21 7.448 21.448 7 22 7 z M 15 9 C 18.309 9 21 11.691 21 15 C 21 18.309 18.309 21 15 21 C 11.691 21 9 18.309 9 15 C 9 11.691 11.691 9 15 9 z M 15 11 A 4 4 0 0 0 11 15 A 4 4 0 0 0 15 19 A 4 4 0 0 0 19 15 A 4 4 0 0 0 15 11 z">
                        </path>
                    </svg>
                </a>
            @endif
            @if ($settings->linkedin_url)
                <a href="{{ $settings->linkedin_url }}" target="_blank" class="link link-hover"
                    aria-label="Linkedin">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 fill-current" viewBox="0 0 30 30">
                        <path
                            d="M24,4H6C4.895,4,4,4.895,4,6v18c0,1.105,0.895,2,2,2h18c1.105,0,2-0.895,2-2V6C26,4.895,25.105,4,24,4z M10.954,22h-2.95 v-9.492h2.95V22z M9.449,11.151c-0.951,0-1.72-0.771-1.72-1.72c0-0.949,0.77-1.719,1.72-1.719c0.948,0,1.719,0.771,1.719,1.719 C11.168,10.38,10.397,11.151,9.449,11.151z M22.004,22h-2.948v-4.616c0-1.101-0.02-2.517-1.533-2.517 c-1.535,0-1.771,1.199-1.771,2.437V22h-2.948v-9.492h2.83v1.297h0.04c0.394-0.746,1.356-1.533,2.791-1.533 c2.987,0,3.539,1.966,3.539,4.522V22z">
                        </path>
                    </svg>
                </a>
            @endif
            @if ($settings->facebook_url)
                <a href="{{ $settings->facebook_url }}" target="_blank" class="link link-hover"
                    aria-label="Facebook">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 fill-current" viewBox="0 0 30 30">
                        <path
                            d="M 15.003906 3 C 8.3739062 3 3 8.3721875 3 14.998047 L 3 20.001953 C 3 26.627813 8.3739062 31.998047 15.003906 31.998047 C 21.632813 31.998047 27.003906 26.627813 27.003906 20.001953 L 27.003906 14.998047 C 27.003906 8.3721875 21.632813 3 15.003906 3 z M 17.001953 14.998047 L 17.001953 18.998047 L 14.001953 18.998047 L 14.001953 14.998047 L 12.501953 14.998047 L 12.501953 12.998047 L 14.001953 12.998047 L 14.001953 11.998047 C 14.001953 10.898047 14.298828 9.9980469 16.001953 9.9980469 L 17.501953 9.9980469 L 17.501953 12.998047 L 16.001953 12.998047 C 15.448828 12.998047 15.001953 13.445922 15.001953 13.998047 L 15.001953 14.998047 z">
                        </path>
                    </svg>
                </a>
            @endif
            @if ($settings->x_url)
                <a href="{{ $settings->x_url }}" target="_blank" class="link link-hover" aria-label="X">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 fill-current" viewBox="0 0 30 30">
                        <path
                            d="M 15.003906 3 C 8.3739062 3 3 8.3721875 3 14.998047 L 3 20.001953 C 3 26.627813 8.3739062 31.998047 15.003906 31.998047 C 21.632813 31.998047 27.003906 26.627813 27.003906 20.001953 L 27.003906 14.998047 C 27.003906 8.3721875 21.632813 3 15.003906 3 z M 19.001953 22 L 17.001953 22 L 13.001953 15.998047 L 9.0019531 22 L 7.0019531 22 L 12.001953 13.998047 L 8.0019531 7 L 10.001953 7 L 13.001953 12.998047 L 16.001953 7 L 18.001953 7 L 14.001953 13.998047 L 19.001953 22 z">
                        </path>
                    </svg>
                </a>
            @endif
        </div>
    </nav>
</footer>
<footer class="footer footer-horizontal footer-center bg-neutral text-accent-content p-5">
    <aside>
        <svg width="50" height="50" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd"
            clip-rule="evenodd" class="inline-block fill-current">
            <path
                d="M22.672 15.226l-2.432.811.841 2.515c.33 1.019-.209 2.127-1.23 2.456-1.15.325-2.148-.321-2.463-1.226l-.84-2.518-5.013 1.677.84 2.517c.391 1.203-.434 2.542-1.831 2.542-.88 0-1.601-.564-1.86-1.314l-.842-2.516-2.431.809c-1.135.328-2.145-.317-2.463-1.229-.329-1.018.211-2.127 1.231-2.456l2.432-.809-1.621-4.823-2.432.808c-1.355.384-2.558-.59-2.558-1.839 0-.817.509-1.582 1.327-1.846l2.433-.809-.842-2.515c-.33-1.02.211-2.129 1.232-2.458 1.02-.329 2.13.209 2.461 1.229l.842 2.515 5.011-1.677-.839-2.517c-.403-1.238.484-2.553 1.843-2.553.819 0 1.585.509 1.85 1.326l.841 2.517 2.431-.81c1.02-.33 2.131.211 2.461 1.229.332 1.018-.21 2.126-1.23 2.456l-2.433.809 1.622 4.823 2.433-.809c1.242-.401 2.557.484 2.557 1.838 0 .819-.51 1.583-1.328 1.847m-8.992-6.428l-5.01 1.675 1.619 4.828 5.011-1.674-1.62-4.829z">
            </path>
        </svg>
        <p class="font-bold text-md">
            Desenvolvido por
            <br />
            Ronaldo P H Campos
        </p>
    </aside>
    <nav>
        <div class="grid grid-flow-col gap-4">
            <a href="https://github.com/ronaldophc" target="_blank" class="link link-hover" aria-label="Github">
                <svg class="fill-current" width="36" height="36" viewBox="0 0 24 24" role="img"
                    xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12">
                    </path>
                </svg>

            </a>
            <a href="https://www.linkedin.com/in/ronaldophc/" target="_blank" class="link link-hover">
                <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 50 50"
                    class="fill-current">
                    <path
                        d="M41,4H9C6.24,4,4,6.24,4,9v32c0,2.76,2.24,5,5,5h32c2.76,0,5-2.24,5-5V9C46,6.24,43.76,4,41,4z M17,20v19h-6V20H17z M11,14.47c0-1.4,1.2-2.47,3-2.47s2.93,1.07,3,2.47c0,1.4-1.12,2.53-3,2.53C12.2,17,11,15.87,11,14.47z M39,39h-6c0,0,0-9.26,0-10 c0-2-1-4-3.5-4.04h-0.08C27,24.96,26,27.02,26,29c0,0.91,0,10,0,10h-6V20h6v2.56c0,0,1.93-2.56,5.81-2.56 c3.97,0,7.19,2.73,7.19,8.26V39z">
                    </path>
                </svg>
            </a>
            <a href="https://www.instagram.com/ronaldohortmann" target="_blank" class="link link-hover">
                <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 50 50"
                    class="fill-current">
                    <path
                        d="M 16 3 C 8.83 3 3 8.83 3 16 L 3 34 C 3 41.17 8.83 47 16 47 L 34 47 C 41.17 47 47 41.17 47 34 L 47 16 C 47 8.83 41.17 3 34 3 L 16 3 z M 37 11 C 38.1 11 39 11.9 39 13 C 39 14.1 38.1 15 37 15 C 35.9 15 35 14.1 35 13 C 35 11.9 35.9 11 37 11 z M 25 14 C 31.07 14 36 18.93 36 25 C 36 31.07 31.07 36 25 36 C 18.93 36 14 31.07 14 25 C 14 18.93 18.93 14 25 14 z M 25 16 C 20.04 16 16 20.04 16 25 C 16 29.96 20.04 34 25 34 C 29.96 34 34 29.96 34 25 C 34 20.04 29.96 16 25 16 z">
                    </path>
                </svg>
            </a>
        </div>
    </nav>
</footer>
